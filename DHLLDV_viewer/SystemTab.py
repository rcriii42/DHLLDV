...
setups = {"CSD Example w/ Ladder Pump": Pipeline(name="CSD with Ladder Pump",
                                                 pipe_list=[Pipe(name='Entrance', diameter=0.85, length=0,
                                                            ...
                                                            Pipe('LP Suction',
                                                                 diameter=0.86,
                                                                 length=ladder_pump_location,
                                                                 total_K=0.1,
                                                                 elev_change=LP_elev-dig_elev),
                                                            copy.copy(Ladder_Pump),
                                                            Pipe('LP Discharge',
                                                                 diameter=0.86,
                                                                 length=ladder_length - ladder_pump_location,
                                                                 total_K=0.15,
                                                                 elev_change=MP_elev-LP_elev),
...
def system_panel(PL):
    ...
    HQ_plot.line('Q', 'Hpump_l', source=im_source,
                 ...
                 name='Pump Water')
    HQ_plot.extra_x_ranges = {'vel_range': Range1d(pipeline.slurry.vls_list[0]*unit_convs['len'],
    ...
    HQ_plot.xaxis[0].axis_label = f'Velocity ({unit_labels["vel"]} in {pipeline.slurry.Dp*unit_convs["dia"]:0.1f} ' \
    ...
    def pipe_panel(pipeline, i):
        ...
        if isinstance(pipe, Pipe):
            ...
            row1 = row(TextInput(title="Pump", value=pipe.name, width=150, background='lightblue'),
                       TextInput(title=f"Suction ({unit_labels['dia']})",
                                 value=f"{pipe.suction_dia*unit_convs['dia']:0.1f}",
                                 width=76, background='lightblue'),
                       TextInput(title=f"Discharge ({unit_labels['dia']})",
                                 value=f"{pipe.disch_dia*unit_convs['dia']:0.1f}",
                                 width=76, background='lightblue'),
                       TextInput(title=f"Impeller ({unit_labels['dia']})",
                                 value=f"{pipe.design_impeller*unit_convs['dia']:0.1f}",
                                 width=76, background='lightblue'),
                       TextInput(title=f"Power ({unit_labels['power']})",
                                 value=f"{pipe.avail_power*unit_convs['power']:0.0f}",
                                 width=76, background='lightblue'),)
    ...
    def pipeline_totals(pipeline):
        ...
        pipe_totals = row(TextInput(title="#", value=f'{pipeline.num_pipesections:3d}', width=45, disabled=True),
                          ...
                          TextInput(title=f"Disch Dia ({unit_labels['dia']})",
                                    value=f"{pipeline.pipesections[-1].diameter*unit_convs['dia']:0.1f}",
                                    width=76, disabled=True),
                          TextInput(title=f"Length ({unit_labels['len']})",
                                    value=f"{pipeline.total_length*unit_convs['len']:0.0f}",
                                    width=76, disabled=True),
                          TextInput(title="Fitting K (-)", value=f"{pipeline.total_K:0.2f}", width=76, disabled=True),
                          TextInput(title=f"Delta z ({unit_labels['len']})",
                                    value=f"{lift*unit_convs['len']:0.1f}",
                                    width=76, disabled=True),)
        elevations = row(TextInput(title=f"Dig Depth ({unit_labels['len']})",
                                   value=f"{dig_depth*unit_convs['len']:0.1f}",
                                   width=100, disabled=True),
                         TextInput(title=f"Discharge Elevation ({unit_labels['len']})",
                                   value=f"{pipeline.total_lift * unit_convs['len']:0.1f}",
                                   width=100, disabled=True))
        ...
        pump_totals = row(TextInput(title="#", value=f'{pipeline.num_pumps:3d}', width=45, disabled=True),
                          ...
                          TextInput(title="Head Water",
                                    value=f"{shutoff_head[2]*unit_convs['len']:0.0f}",
                                    width=95, disabled=True),
                          TextInput(title="Head Slurry",
                                    value=f"{shutoff_head[3]*unit_convs['len']:0.0f}",
                                    width=95, disabled=True),
                          TextInput(title=f"Power ({unit_labels['power']})",
                                    value=f"{pipeline.total_power*unit_convs['power']:0.0f}",
                                    width=95, disabled=True),)
    ...
    opcol = column(Spacer(background='lightblue', height=5, margin=(5, 0, 5, 0)),
                   ...
                   row(TextInput(title=f"Q {unit_labels['flow']}", value=f'{qimin*unit_convs["flow"]:0.2f}',
                                 width=95, disabled=True),
                       TextInput(title=f"v\u2098 ({unit_labels['vel']})",
                                 value=f'{pipeline.pipesections[-1].velocity(qimin)*unit_convs["len"]:0.1f}',
                                 width=95, disabled=True),
                       TextInput(title=f"H ({unit_labels['len']})",
                                 value=f'{pipeline.calc_system_head(qimin)[0]*unit_convs["len"]:0.1f}',
                                 width=95, disabled=True)
                       ),
